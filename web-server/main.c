...
void doRoute(int new_socket, char* request){
  ...
  if(strncmp(request, "/", 10) == 0){
    ...
    if (strstr(request, ".html") != NULL) {
      ...
      doWrite(new_socket,"Content-Type: test/css\r\n\r\n",28,request);
    }
    else if(strstr(request,".mp4")!=NULL){
      doWrite(new_socket,"Content-Type:video/mp4\r\n\r\n",28,request);
    }
...
int main()
{
  ...
  address.sin_port = htons(8080); //host byte to network byte
